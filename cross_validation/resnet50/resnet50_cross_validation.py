...
# ---------------------------
# Model Factory: ResNet-50 (prefer timm, fallback torchvision)
# ---------------------------
def build_resnet50(num_classes: int) -> Tuple[nn.Module, str]:
    ...
    # replace fc
    if hasattr(tv_model, 'fc') and isinstance(tv_model.fc, nn.Linear):
        in_f = tv_model.fc.in_features
        tv_model.fc = nn.Linear(in_f, num_classes)
    else:
        ...
        for name, mod in reversed(list(tv_model.named_modules())):
            if isinstance(mod, nn.Linear):
                ...
                break
        if not replaced:
            raise RuntimeError('Cannot replace final Linear for torchvision resnet50')
...
def run_one_fold(fold_id: int, df: pd.DataFrame, train_idx: np.ndarray, test_idx: np.ndarray,
    ...
    model, backend = build_resnet50(num_classes)
